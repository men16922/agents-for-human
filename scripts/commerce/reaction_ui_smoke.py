...
def inspect_live(directory, services, worker_errors, seconds):
    """Keep verified live services available; the record describes this window only."""
    if type(seconds) is not int or not 0 <= seconds <= 3600:
        raise ValueError("Inspection seconds must be 0 (until Ctrl+C) through 3600")
    record = {
        "state": "OPEN", "url": "http://127.0.0.1:15173",
        "scope": "live-medusa-with-scripted-sdk-buyer",
        "real_model_calls": 0, "human_participation_measured": False,
        "started_at": time.time(), "reason": None,
    }
    marker = directory / "inspection.json"
    deadline = time.monotonic() + seconds if seconds else None
    write(marker, record)
    print(
        "Inspection ready: http://127.0.0.1:15173\n"
        "Real local Medusa transaction; deterministic SDK buyer; no paid model.\n"
        f"Evidence: {directory}\n"
        "Ctrl+C ends inspection and exports the final independent evidence.\n"
        "Stop the separate make commerce terminal afterwards.",
        flush=True,
    )
    try:
        while True:
            if worker_errors or any(p.poll() is not None for p in services):
                raise RuntimeError("Inspection service exited; see private logs")
            if deadline is not None and time.monotonic() >= deadline:
                record["reason"] = "BOUNDED_WINDOW_FINISHED"
                break
            time.sleep(0.2)
    except KeyboardInterrupt:
        record["reason"] = "OPERATOR_INTERRUPT"
    except Exception:
        record.update(state="FAILED", reason="SERVICE_FAILURE")
        raise
    finally:
...
def main(inspect_seconds=None):
    ...
    try:
        ...
        if inspect_seconds is not None:
            report["inspection"] = inspect_live(
                directory, [api, web, *session.processes], worker_errors, inspect_seconds
            )
        stopping.set()
        worker.join(5)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--inspect", nargs="?", type=int, const=0, metavar="SECONDS",
        help="Keep the live view open after checks; omit seconds to wait for Ctrl+C",
    )
    args = parser.parse_args()
    if args.inspect is not None and not 0 <= args.inspect <= 3600:
        parser.error("--inspect must be between 0 and 3600 seconds")
    main(args.inspect)
