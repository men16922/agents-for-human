...
const cloud = existsSync(join(directory, 'build-report.json'));
const video = await readFile(join(directory, cloud ? 'rehearsal-demo.mp4' : 'rehearsal-draft.mp4'));
const captions = await readFile(join(directory, 'captions.vtt'));
const report = JSON.parse(await readFile(join(directory, cloud ? 'build-report.json' : 'report.json'), 'utf8'));
const expectedDuration = cloud ? report.seconds : report.duration_seconds;
const seeks = [0.02,0.12,0.26,0.43,0.51,0.65,0.81,0.96].map(f => Math.floor(f * expectedDuration));
const html = '<!doctype html><html lang="en"><meta charset="utf-8"><style>body{margin:24px;background:#101c2c;color:white;font:18px Arial}video{width:1056px;height:594px;display:block}p{margin:18px 0}</style><body><video muted controls preload="auto"><source src="/video" type="video/mp4"><track default kind="subtitles" src="/captions" srclang="en" label="English"></video><p>LOCAL PLAYBACK CHECK · Inspect the source labels and recorded outcomes</p></body></html>';
...
try {
  ...
  await writeFile(join(directory, 'playback-check.json'), JSON.stringify({ passed: true, metadata, track, errors, seeks: seeks }, null, 2) + '\n');
