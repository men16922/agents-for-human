...
try{
 ...
 await page.getByRole('button',{name:'증거 다시 검증'}).click();await expect(page.getByTestId('evidence-result')).toContainText('증거 시점 목표 완료',{timeout:15000});
 ...
 expect(execution.status).toBe('SEALED');expect(evidence.verdict.status).toBe('COMPLETE');expect(evidence.run_id).toBe(execution.run_id);expect(requests.every(r=>r.method==='GET'&&!r.auth)).toBe(true);expect(errors).toEqual([]);expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);
 await page.screenshot({path:join(dir,'final.png'),fullPage:true});await save('browser-report',{passed:true,scope:'actual-nova-medusa-ui',execution,evidence,states,requests,errors});await page.waitForTimeout(3000);
