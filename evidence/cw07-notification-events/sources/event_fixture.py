...
def apply_control_event(session, event):
    import httpx
    ...
    path = f"/admin/runs/{session.run['binding']['run_id']}/faults/{route}"
    body = {"event_id": event["id"], "delay_ms": event["delay_ms"]}
    if route == "scheduled-notification":
        body.update(kind=event["kind"], copies=event["copies"])
...
def apply_event(session, event):
    if event["kind"] in {"payment_response_delay", "delivery_notification", "notification_replay"}:
        return apply_control_event(session, event)
...
def export_events(session):
    ...
    if any(
        ...
        for e in session.case["events"]
    ):
        from pathlib import Path

        from rehearsal.commerce.seller import read_rows

        directory = Path(session.run["directory"])
        result["clock_started_at"] = json.loads(
            read_rows(directory / "medusa.sqlite3", "binding")[0]["data"]
        )["started_at"]
        for table in ("notification_schedules", "scheduled_deliveries"):
            result[table] = [
                r
                for r in read_rows(directory.parent / "observations.sqlite3", table)
                if r["run_id"] == session.run["binding"]["run_id"]
            ]
    return result
