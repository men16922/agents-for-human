    ...
    try:
        for cid, method in [("timed", "B0"), ("pending", "B0"), ("timed", "B1")]:
            ...
            try:
                ...
                assert selected["external_transaction_verified"] == (method == "B0"), (
        ...
        result["known_sdk_failure"] = (
            "stale A quote retained after refresh failure; missing order id; last call usage unresolved"
        )
        try:
            external_batch.learn(directory, "timed-B2-r1", fixture_factory)
        except ValueError as exc:
            assert "UNRESOLVED_ATTEMPT" in str(exc)
