...
try{
 await page.goto('http://127.0.0.1:15173');await expect(page.getByTestId('connection')).toHaveText('Live connection',{timeout:20000});await page.screenshot({path:join(dir,'initial.png')});await save('initial',{at:Date.now()/1000,stock:await page.getByTestId('stock-A-tent').innerText()});
 await expect.poll(async()=>{try{await access(join(dir,'finished.json'));return true;}catch{return false;}},{timeout:150000}).toBe(true);
 await expect(page.getByRole('region',{name:'Executor reaction record'}).getByText('Sealed record · file integrity verified')).toBeVisible({timeout:15000});
 await page.getByRole('button',{name:'Reverify evidence'}).click();await expect.poll(async()=>{const r=await(await page.request.get('http://127.0.0.1:15173/api/observer/evidence')).json();return r.status;},{timeout:15000}).toBe('VERIFIED');
 ...
 await page.getByRole('region',{name:'Independent evidence'}).scrollIntoViewIfNeeded();await page.screenshot({path:join(dir,'final.png')});await save('browser-report',{passed:true,scope:'actual-nova-medusa-ui',execution,evidence,states,requests,errors});await page.waitForTimeout(3000);
