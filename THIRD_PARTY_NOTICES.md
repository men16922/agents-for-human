...
- No OpenMMO code or game assets are incorporated. The investigation and upstream links are research references only.
- `assets/proposal/rehearsal-world.png` and `rehearsal-dashboard.png` are AI-generated proposal concepts. Prompts and provenance are in `assets/proposal/IMAGEGEN.md`; they are not working-product evidence.
- The overnight runner remains in the locally installed plugin. Only per-repository configuration and documents are included here.
...
- AWS architecture icons under `assets/architecture/aws/` are unmodified assets from the official 2026-07-31 [AWS Architecture Icons package](https://aws.amazon.com/architecture/icons/). They are embedded in the editable draw.io and SVG diagrams. AWS owns these service icons and trademarks; the project MIT license does not relicense them. Exact source paths are recorded in `assets/architecture/aws/sources.json`.
