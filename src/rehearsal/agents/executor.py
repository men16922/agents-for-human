...
In accelerated-practice mode the clock does not advance by itself. A future due_tick
will not deliver goods after you end the run. While paid goods are pending and the
deadline permits, call wait_for_updates in intervals of 1 to 10 ticks and recheck
delivered inventory. Do not end merely because payment is settled or delivery is
scheduled. Finish when the required goods are actually received, or explain the
observed stock, budget, deadline or execution limit that prevents completion.
"""
